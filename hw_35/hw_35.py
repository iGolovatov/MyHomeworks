...
def initialize_db():
    try:
        ...
        for master in Master.select():
            print(f"{master.id}: {master.last_name} {master.first_name} (Телефон: {master.phone})")

        print("\n=== Услуги ===")
        for service in Service.select():
            print(f"{service.id}: {service.title} - {service.price} руб. "
                  f"({service.description[:30] if service.description else 'Без описания'})")

        print("\n=== Заявки ===")
        for appointment in Appointment.select():
            services = [as_entry.service.title for as_entry in AppointmentService.filter(appointment=appointment)]
            print(f"{appointment.id}: {appointment.client_name} ({appointment.client_phone}) "
                  f"у мастера {appointment.master.last_name} {appointment.master.first_name} "
                  f"на {appointment.date.strftime('%d.%m.%Y %H:%M')} (Услуги: {', '.join(services)})")
